...
export class PlaceView extends Gtk.Box {
    ...
    _calculatePixelSizeForShield(paintable, pixelHeight) {
        ...
        if (Math.max(intrinsicWidth, intrinsicHeight) < pixelHeight)
            return Math.max(intrinsicWidth, intrinsicHeight);
        else
            return Math.max((intrinsicWidth / intrinsicHeight) * pixelHeight,
                            pixelHeight);
    }
    ...
    updatePlaceDetails() {
        ...
        if (place.source) {
            ...
            if (shieldPaintables.length > 0) {
                ...
                log(`height: ${shieldPaintables[0].get_intrinsic_height()}, width: ${shieldPaintables[0].get_intrinsic_width()}`);
                if (shieldPaintables.length > 1) {
