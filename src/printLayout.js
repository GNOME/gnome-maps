...
var PrintLayout = GObject.registerClass({
    ...
    _drawInstruction(width, height, turnPoint) {
        ...
        if (iconName) {
            Utils.debug('adding icon to surface: ' + iconName);
            let pixbuf = theme.load_icon(iconName, height, 0);

            Utils.debug('pixbuf: ' + pixbuf);

            Gdk.cairo_set_source_pixbuf(cr, pixbuf,
                                        this._rtl ? width - height - 1 : 0, 0);
        ...
        layout.set_alignment(Pango.Alignment.LEFT); // TODO: do we need to flip this for RTL?
        ...
        if (turnPoint.distance > 0) {
            ...
            layout.set_font_description(desc);
            layout.set_alignment(Pango.Alignment.RIGHT); // TODO: RTL?
            cr.moveTo(this._rtl ? 0 : width - height * 2, 0);
            PangoCairo.layout_path(cr, layout);
            cr.setSourceRGB(0.0, 0.0, 0.0);
            cr.fill();
        }
