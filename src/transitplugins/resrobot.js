...
var Resrobot = class Resrobot {
    ...
    _processItineraries(itineraries) {
        ...
        let distance = endLoc.get_distance_from(startLoc) * 1000;

        if (!this._extendPrevious && distance <= 2000) {
            GraphHopperTransit.fetchWalkingRoute([start, end], (route) => {
                ...
                this._addWalkingToItinerariesRecursive(itineraries, 0);
            });
        } else {
            this._addWalkingToItinerariesRecursive(itineraries, 0);
        }
    ...
    _addWalkingToItinerariesRecursive(itineraries, index) {
        if (index === itineraries.length) {
            Utils.debug('updating with new itineraries');
            ...
            this._addWalkingToLegsRecursive(itinerary.legs, 0, () => {
                this._addWalkingToItinerariesRecursive(itineraries, index + 1);
            });
        }
    }

    _addWalkingToLegsRecursive(legs, index, callback) {
            ...
            if (!leg.transit) {
                ...
                GraphHopperTransit.fetchWalkingRoute([from, to], (route) => {
                    if (route) {
                        ...
                        Utils.debug('got walking route with duration: ' + duration);
                        Utils.debug('original duration: ' + leg.duration);
                        if (index === 0 || index === legs.length - 1 ||
                            duration <= leg.duration) {
                            Utils.debug('Adding walking route');
                    ...
                    this._addWalkingToLegsRecursive(legs, index + 1, callback);
                });
            } else {
                this._addWalkingToLegsRecursive(legs, index + 1, callback);
            }
