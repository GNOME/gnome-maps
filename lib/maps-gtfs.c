...
static gboolean
init_db (MapsGTFS *gtfs, GError **error)
{
  MapsGTFSPrivate *priv = maps_gtfs_get_instance_private (gtfs);
  ...
  sqlite3_exec (priv->db, "VACUUM", NULL, NULL, &msg);
  sqlite3_db_config (priv->db, SQLITE_DBCONFIG_RESET_DATABASE, 0, 0);

  if (msg) {
    *error = g_error_new (MAPS_GTFS_ERROR, 0, "Failed to reset DB: %s",
                          msg);
    return FALSE;
  }
