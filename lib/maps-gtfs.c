                            ...
                            gfloat distance)
{
  ...
  const gchar *sql = "SELECT stop_id, stop_code, stop_name, stop_desc, " \
                            ...
                            "stop_timezone, DISTANCE(stop_lat, stop_lon, ?, ?) d " \
                     "FROM stops " \
  ...
  while (!done) {
    ...
    case SQLITE_ROW:
      ...
      code = sqlite3_column_text (p_stmt, 1);
      name = sqlite3_column_text (p_stmt, 2);
      desc = sqlite3_column_text (p_stmt, 3);
      lat = sqlite3_column_double (p_stmt, 4);
      lon = sqlite3_column_double (p_stmt, 5);
      location_type = sqlite3_column_int (p_stmt, 6);
      parent_station = sqlite3_column_text (p_stmt, 7);
      d = sqlite3_column_double (p_stmt, 9);

      g_debug("Stop: %s, %s, %f", id, name, d);
      break;
    case SQLITE_DONE:
      done = TRUE;
      break;
