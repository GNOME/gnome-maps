...
process_stop (MapsGTFS *gtfs, GHashTable *entry, sqlite3_stmt *p_stmt)
{
  ...
  if (location_type)
    sqlite3_bind_int (p_stmt, 7, g_ascii_strtoull (location_type, NULL, 10));
  else
    sqlite3_bind_null (p_stmt, 7);

  if (timezone)
    sqlite3_bind_text (p_stmt, 8, timezone, -1, SQLITE_TRANSIENT);
  else
    sqlite3_bind_null (p_stmt, 8);
...
process_route (MapsGTFS *gtfs, GHashTable *entry, sqlite3_stmt *p_stmt)
{
  ...
  sqlite3_bind_int (p_stmt, 5, g_ascii_strtoull (route_type, NULL, 10));
  sqlite3_bind_text (p_stmt, 6, color, -1, SQLITE_TRANSIENT);
  sqlite3_bind_text (p_stmt, 7, text_color, -1, SQLITE_TRANSIENT);
