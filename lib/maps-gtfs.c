...
process_agency (MapsGTFS *gtfs, GHashTable *entry, sqlite3_stmt *p_stmt)
{
  ...
  gchar *url = g_hash_table_lookup (entry, "agency_url");
...
static gboolean
process_stop (MapsGTFS *gtfs, GHashTable *entry, sqlite3_stmt *p_stmt)
{
  ...
  gchar *zone_id = g_hash_table_lookup (entry, "zone_id");
  gchar *url = g_hash_table_lookup (entry, "stop_url");
  ...
  gchar *wheelchair_boarding = g_hash_table_lookup (entry, "wheelchair_boarding");
  gchar *level_id = g_hash_table_lookup (entry, "level_id");
  gchar *platform_code = g_hash_table_lookup (entry, "platform_code");
...
static gboolean
process_route (MapsGTFS *gtfs, GHashTable *entry, sqlite3_stmt *p_stmt)
{
  ...
  gchar *desc = g_hash_table_lookup (entry, "route_desc");
  gchar *route_type = g_hash_table_lookup (entry, "route_type");
  gchar *url = g_hash_table_lookup (entry, "route_url");
...
static gboolean
process_trip (MapsGTFS *gtfs, GHashTable *entry, sqlite3_stmt *p_stmt)
{
  ...
  sqlite3_bind_text (p_stmt, 6, shape_id, -1, SQLITE_TRANSIENT);
...
static gboolean
process_stop_time (MapsGTFS *gtfs, GHashTable *entry, sqlite3_stmt *p_stmt)
{
  ...
  sqlite3_bind_int (p_stmt, 7, pickup_type_val);
  sqlite3_bind_int (p_stmt, 8, drop_off_type_val);
               ...
               sqlite3_stmt *p_stmt)
{
  ...
  case STOPS:      return process_stop (gtfs, entry, p_stmt);
  case ROUTES:     return process_route (gtfs, entry, p_stmt);
  case TRIPS:      return process_trip (gtfs, entry, p_stmt);
  case STOP_TIMES: return process_stop_time (gtfs, entry, p_stmt);
