                      ...
                      gchar *lang, gchar *phone, gchar *fare_url, gchar *email)
{
  MapsGTFSAgency *agency =
    MAPS_GTFS_AGENCY (g_object_new (MAPS_TYPE_GTFS_AGENCY,
                                    "id", id,
                                    "name", name,
                                    "url", url,
                                    "timezone", timezone,
...
static void
maps_gtfs_agency_set_property (GObject    *object,
                               guint       prop_id,
                               const GValue     *value,
                               GParamSpec *pspec)
{
  ...
  switch (prop_id)
    ...
    case PROP_URL:
      ...
      break;
    case PROP_TIMEZONE:
      priv->timezone = g_value_get_boxed (value);
      break;
    case PROP_LANG:
      priv->lang = g_value_dup_string (value);
      break;
    case PROP_PHONE:
      priv->phone = g_value_dup_string (value);
      break;
    case PROP_FARE_URL:
      priv->fare_url = g_value_dup_string (value);
      break;
    case PROP_EMAIL:
...
maps_gtfs_agency_class_init (MapsGTFSAgencyClass *klass)
{
  ...
  object_class->set_property = maps_gtfs_agency_set_property;

  properties[PROP_ID] =
    g_param_spec_string ("id",
                         "ID", "Unique identifier for agency",
                         NULL,
                         G_PARAM_CONSTRUCT_ONLY|G_PARAM_EXPLICIT_NOTIFY);
